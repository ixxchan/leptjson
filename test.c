...
#define EXPECT_EQ_STRING(expect, actual, actual_len)\
    EXPECT_EQ_BASE(sizeof(expect) -1 == (actual_len) && memcmp(expect, actual, actual_len) == 0, expect, actual, "%s")
#define EXPECT_EQ_INT(expect, actual) EXPECT_EQ_BASE((expect) == (actual), expect, actual, "%d")
#define EXPECT_EQ_SIZE(expect, actual) EXPECT_EQ_BASE((expect) == (actual), (size_t)(expect), actual, "%zu")
#define EXPECT_EQ_DOUBLE(expect, actual) EXPECT_EQ_BASE((expect) == (actual), (double)(expect), actual, "%.17g")
...
static void test_access_string() {
    ...
    lept_set_string(&v, "Hello\0World", 11);
    EXPECT_EQ_SIZE((size_t) 11, lept_get_string_length(&v));
    EXPECT_EQ_STRING("Hello\0World", lept_get_string(&v), lept_get_string_length(&v));
    lept_free(&v);
}
...
static void test_access_array() {
    ...
    for (j = 0; j <= 5; j += 5) {
        lept_set_array(&a, j);
        EXPECT_EQ_SIZE(0, lept_get_array_size(&a));
        EXPECT_EQ_SIZE(j, lept_get_array_capacity(&a));
        for (i = 0; i < 10; i++) {
            lept_set_number(lept_pushback_array_element(&a), i);
        }

        EXPECT_EQ_SIZE(10, lept_get_array_size(&a));
        for (i = 0; i < 10; i++)
            EXPECT_EQ_DOUBLE((double) i, lept_get_number(lept_get_array_element(&a, i)));
    }

    lept_popback_array_element(&a);
    EXPECT_EQ_SIZE(9, lept_get_array_size(&a));
    for (i = 0; i < 9; i++)
        EXPECT_EQ_DOUBLE((double) i, lept_get_number(lept_get_array_element(&a, i)));
    ...
    EXPECT_EQ_SIZE(8, lept_get_array_size(&a));
    for (i = 0; i < 8; i++)
        EXPECT_EQ_DOUBLE((double) i, lept_get_number(lept_get_array_element(&a, i)));

    lept_erase_array_element(&a, 0, 2);
    EXPECT_EQ_SIZE(6, lept_get_array_size(&a));
    for (i = 0; i < 6; i++)
    ...
    lept_shrink_array(&a);
    EXPECT_EQ_SIZE(8, lept_get_array_capacity(&a));
    EXPECT_EQ_SIZE(8, lept_get_array_size(&a));
    for (i = 0; i < 8; i++)
        EXPECT_EQ_DOUBLE((double) i, lept_get_number(lept_get_array_element(&a, i)));
    ...
    lept_clear_array(&a);
    EXPECT_EQ_SIZE(0, lept_get_array_size(&a));
    EXPECT_EQ_SIZE(i, lept_get_array_capacity(&a));   /* capacity remains unchanged */
    lept_shrink_array(&a);
    EXPECT_EQ_SIZE(0, lept_get_array_capacity(&a));
...
static void test_access_object() {
    ...
    for (j = 0; j <= 5; j += 5) {
        lept_set_object(&o, j);
        EXPECT_EQ_SIZE(0, lept_get_object_size(&o));
        EXPECT_EQ_SIZE(j, lept_get_object_capacity(&o));
        for (i = 0; i < 10; i++) {
            char key[2] = "a";
            key[0] += i;
            pv = lept_set_object_value(&o, key, 1);
            lept_init(pv);
            lept_set_number(pv, i);
        }
        EXPECT_EQ_SIZE(10, lept_get_object_size(&o));
        for (i = 0; i < 10; i++) {
            char key[] = "a";
            key[0] += i;
    ...
    index = lept_find_object_index(&o, "j", 1);
    EXPECT_TRUE(index == LEPT_KEY_NOT_EXIST);
    EXPECT_EQ_SIZE(9, lept_get_object_size(&o));
    ...
    index = lept_find_object_index(&o, "a", 1);
    EXPECT_TRUE(index == LEPT_KEY_NOT_EXIST);
    EXPECT_EQ_SIZE(8, lept_get_object_size(&o));

    EXPECT_TRUE(lept_get_object_capacity(&o) > 8);
    lept_shrink_object(&o);
    EXPECT_EQ_SIZE(8, lept_get_object_capacity(&o));
    EXPECT_EQ_SIZE(8, lept_get_object_size(&o));
    for (i = 0; i < 8; i++) {
        char key[] = "a";
        key[0] += i + 1;
    ...
    pv = lept_set_object_value(&o, "World", 5);
    lept_init(pv);
    lept_set_string(pv, "Hello", 5);
    ...
    lept_clear_object(&o);
    EXPECT_EQ_SIZE(0, lept_get_object_size(&o));
    EXPECT_EQ_SIZE(i, lept_get_object_capacity(&o)); /* capacity remains unchanged */
    lept_shrink_object(&o);
    EXPECT_EQ_SIZE(0, lept_get_object_capacity(&o));
...
static void test_parse_number() {
    ...
    TEST_NUMBER((double) 1, "1.0000000000000001");
...
static void test_parse_array() {
    ...
    EXPECT_EQ_SIZE(0, lept_get_array_size(&v));
    lept_free(&v);

    lept_init(&v);
    EXPECT_EQ_INT(LEPT_PARSE_OK, lept_parse(&v, "[ null , false , true , 123 , \"abc\" ]"));
    EXPECT_EQ_INT(LEPT_ARRAY, lept_get_type(&v));
    EXPECT_EQ_SIZE(5, lept_get_array_size(&v));
    ...
    EXPECT_EQ_INT(LEPT_ARRAY, lept_get_type(&v));
    EXPECT_EQ_SIZE(4, lept_get_array_size(&v));
    for (int i = 0; i < 4; ++i) {
        lept_value *p;
        p = lept_get_array_element(&v, i);
        EXPECT_EQ_SIZE(i, lept_get_array_size(p));
        for (int j = 0; j < i; ++j) {
            EXPECT_EQ_DOUBLE(j, lept_get_number(lept_get_array_element(p, j)));
        }
    }
    lept_free(&v);
}
...
static void test_parse_object() {
    ...
    EXPECT_EQ_SIZE(0, lept_get_object_size(&v));
    lept_free(&v);
    ...
    EXPECT_EQ_INT(LEPT_OBJECT, lept_get_type(&v));
    EXPECT_EQ_SIZE(7, lept_get_object_size(&v));
    ...
    EXPECT_EQ_SIZE(3, lept_get_array_size(lept_get_object_value(&v, 5)));
    for (i = 0; i < 3; i++) {
    ...
    EXPECT_EQ_STRING("o", lept_get_object_key(&v, 6), lept_get_object_key_length(&v, 6));
    {
        ...
        for (i = 0; i < 3; i++) {
            ...
            EXPECT_EQ_SIZE(1, lept_get_object_key_length(o, i));
